...
def classify_reasons(df):
    df['EnrollmentGroup'] = df['EnrollmentReasonDesc'].map(enrollment_reason_map).fillna("Unclassified")
    df['WithdrawalGroup'] = df['WithDrawalReasonDesc'].map(withdrawal_reason_map).fillna("Unclassified")
    return df
...
def join_enrollment_withdrawal(enrollment, enrollment_reason , withdrawal_reason):
    ...
    enrol_reasons = pd.merge(enrollment, enrollment_reason [['EnrollmentReasonId','EnrollmentReasonDesc']], on = 'EnrollmentReasonId', how = 'left') 
    # get the withdrawal reason description
    registration = pd.merge(enrol_reasons, withdrawal_reason[['WithdrawalReasonId', 'WithDrawalReasonDesc']], on='WithdrawalReasonId', how='left')
